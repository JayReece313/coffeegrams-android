...
- A **Qodo review** and a **Claude review** (`.github/workflows/claude-code-review.yml`) both run automatically on every PR push — Qodo's has been intermittently unavailable (trial/billing lapses), Claude's is the reliable fallback. Drive findings from whichever ran to **zero** before merging or calling a milestone done. A separate `@claude`-mention workflow (`.github/workflows/claude.yml`) is also available for an on-demand review or question on any PR or issue, distinct from the automatic one.
