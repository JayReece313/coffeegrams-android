...
- A **Qodo review** runs automatically on every PR push. Drive findings to **zero** before merging or calling a milestone done. A separate `@claude`-mention workflow (`.github/workflows/claude.yml`) is also available for an on-demand review or question on any PR or issue, but nothing Claude-side runs automatically.
